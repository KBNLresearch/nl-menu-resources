...
Opening the WARC in pywb again results in 80K captures. However, if we copy the index.html from `var/www/www.nl-menu.nl/nlmenu.nl/` to the domain root before running warcit, this produces a single-capture WARC that renders in pywb (although oddly it is over 60 MB smaller than the WARC that is generated by wget!).
